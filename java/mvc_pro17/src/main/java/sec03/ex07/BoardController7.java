	...
	protected void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (action == null) {
				...
				for (int _articleNO : articleNOList) {//답글의 답글까지 삭제
				...
				PrintWriter pw = response.getWriter();
				pw.print("<script>" + " alert('새글을 삭제했습니다.');" + " location.href='" + request.getContextPath() + "/board/listArticles.do';" + "</script>");
				return;
				...
				if(imageFileName != null && imageFileName.length() != 0) {
					...
					File destDir = new File(ARTICLE_IMAGE_REPO +"\\" + articleNO); //참조 주소 변수 생성
					destDir.mkdirs();
					FileUtils.moveFileToDirectory(srcFile, destDir, true); 
				}
				PrintWriter pw = response.getWriter();
